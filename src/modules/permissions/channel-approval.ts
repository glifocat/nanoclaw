...
export async function createNewAgentGroup(
  ...
  await initGroupFilesystem(ag, { provider: 'opencode', instructions: modelProvider.instructions ?? undefined });
  await updateContainerConfigScalars(ag.id, {
    provider: 'opencode',
    model: model.id,
    delivery_mode:
      modelProvider.delivery_mode === 'envelope' || modelProvider.delivery_mode === 'tools-only'
        ? modelProvider.delivery_mode
        : null,
  });
  await updateContainerConfigJson(ag.id, 'provider_settings', {
    opencode: {
