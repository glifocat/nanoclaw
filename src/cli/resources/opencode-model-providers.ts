...
function validateProvider(values: Record<string, unknown>): void {
  ...
  const deliveryMode = optionalString(values.delivery_mode);
  if (deliveryMode && deliveryMode !== 'envelope' && deliveryMode !== 'tools-only') {
    throw new Error('delivery_mode must be envelope or tools-only');
  }
  values.delivery_mode = deliveryMode ?? null;
...
registerResource({
  ...
  columns: [
    ...
    },
    {
      name: 'delivery_mode',
      type: 'string',
      description: 'Optional per-group delivery mode.',
      enum: ['envelope', 'tools-only'],
      updatable: true,
    },
    {
      name: 'instructions',
