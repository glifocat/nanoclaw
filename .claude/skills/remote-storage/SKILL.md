...
First, check if there are existing rclone remotes that connect to the same server:

```bash
rclone listremotes 2>/dev/null
```

If there are existing remotes, use AskUserQuestion:
- **question:** "I found existing rclone remotes. How do you want to configure credentials for `nanoclaw-{name}`?"
- **header:** "Credentials"
- **options:**
  - label: "Copy from existing (Recommended)", description: "Copy credentials from an existing remote that connects to the same server"
  - label: "Configure new", description: "Set up credentials from scratch via interactive rclone config"

**If "Copy from existing":**

Read the rclone config file to copy the credentials section:

```bash
cat ~/.config/rclone/rclone.conf
```

Find the section for the existing remote that connects to the same URL. Create the new remote by writing a new section to the config file with the same `user`, `pass`, and `vendor` values but the new name and URL:
...
type = webdav
url = {url}
vendor = {vendor_from_existing}
user = {user_from_existing}
pass = {encrypted_pass_from_existing}
EOF
```

**Important:** The `pass` value in `rclone.conf` is already obscured/encrypted — copy it as-is, do NOT attempt to decrypt or re-obscure it.
...
**After either path**, verify the remote works:
...
```bash
npx tsx -e "import { initDatabase, getAllRegisteredGroups } from './src/db.ts'; initDatabase(); const g = getAllRegisteredGroups(); for (const [jid, group] of Object.entries(g)) { console.log(group.folder + (group.isMain ? ' (main)' : '')); }"
```
