...
> These steps run unconditionally on every invocation; all are individually idempotent. If you carry local patches to `src/channels/slack.ts`, note that step 2 overwrites the file from `origin/channels`.
...
If `.env` already contains `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET`, the channel is already configured — skip to **Next Steps**. Otherwise, walk through the steps below.
