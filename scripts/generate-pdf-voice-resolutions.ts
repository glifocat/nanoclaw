/**
 * Generate rerere-compatible resolution files for the pdf-reader + voice-transcription
 * skill combination.
 *
 * Both skills modify src/channels/whatsapp.ts and src/channels/whatsapp.test.ts.
 * When CI applies pdf-reader first, then voice-transcription, the three-way merge
 * fails on those shared files. This script pre-computes the resolutions.
 *
...
// The files that conflict when applying voice-transcription after pdf-reader
const conflictFiles = [
  'src/channels/whatsapp.ts',
  'src/channels/whatsapp.test.ts',
];

// "Ours" = pdf-reader (first skill applied)
const pdfReaderModify = '.claude/skills/add-pdf-reader/modify';
// "Theirs" = voice-transcription (second skill applied)
const voiceTranscriptionModify = '.claude/skills/add-voice-transcription/modify';
const shippedResDir = path.join(
  projectRoot,
  '.claude',
  'resolutions',
  'add-pdf-reader+add-voice-transcription',
);
...
for (const relPath of conflictFiles) {
  ...
  const oursPath = path.join(projectRoot, pdfReaderModify, relPath);
  const theirsPath = path.join(projectRoot, voiceTranscriptionModify, relPath);

  // Resolution = the correct combined file. Read from backup copy.
  ...
  fileHashes[relPath] = {
    ...
    current: sha256(oursPath), // "ours" = pdf-reader's modify (current state after first skill)
    skill: sha256(theirsPath), // "theirs" = voice-transcription's modify (the skill being applied)
  };
...
// Write meta.yaml
const meta = {
  skills: ['add-pdf-reader', 'add-voice-transcription'],
  apply_order: ['add-pdf-reader', 'add-voice-transcription'],
...
console.log(
  `\nGenerated ${results.length} resolution(s) in .claude/resolutions/add-pdf-reader+add-voice-transcription/`,
);
