...
//   reminders-cli add_item <list_name> <title> [--notes <text>] [--due <ISO8601>] [--priority <none|low|medium|high>]
//   reminders-cli update_item <list_name> <title> [--new-title <t>] [--new-notes <n>] [--new-due <d>] [--new-priority <p>]
...
func addItem(listName: String, title: String, notes: String?, dueDate: String?, priority: String?) {
    guard let cal = findCalendar(listName) else { fail("List not found: \(listName)") }
...
func updateItem(listName: String, title: String, newTitle: String?, newNotes: String?, newDueDate: String?, newPriority: String?) {
    guard let cal = findCalendar(listName) else { fail("List not found: \(listName)") }
...
case "add_item":
    guard args.count >= 3 else { fail("Usage: add_item <list_name> <title> [--notes <text>] [--due <ISO8601>] [--priority <none|low|medium|high>]") }
    var notes: String?
    var dueDate: String?
    var priority: String?
    var i = 3
    ...
    addItem(listName: args[1], title: args[2], notes: notes, dueDate: dueDate, priority: priority)

case "update_item":
    guard args.count >= 3 else { fail("Usage: update_item <list_name> <title> [--new-title <t>] [--new-notes <n>] [--new-due <d>] [--new-priority <none|low|medium|high>]") }
    var newTitle: String?
    var newNotes: String?
    ...
    if newTitle == nil && newNotes == nil && newDueDate == nil && newPriority == nil { fail("No update fields provided") }
    updateItem(listName: args[1], title: args[2], newTitle: newTitle, newNotes: newNotes, newDueDate: newDueDate, newPriority: newPriority)
